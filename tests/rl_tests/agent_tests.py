...
class TestAgent(unittest.TestCase):
    ...
    def test_interaction_with_environment(self):
        env = TradingEnvironment()
        agent = DDPGAgent(state_dim=len(env.observations),
                        action_dim=len(env.actions),
                        max_action=max(env.actions),
                        lstm_hidden_dim=256,
                        num_lstm_layers=2,
                        dropout_rate=0.5,
    ...
    def test_action_range(self):
        ...
        agent = DDPGAgent(state_dim=len(initial_state),
                        action_dim=len(env.action_space),
                        max_action=max(env.action_space),
                        lstm_hidden_dim=256,
                        num_lstm_layers=2,
                        dropout_rate=0.5,
                        max_buffer_size=1000000)
        ...
        for _ in range(100):
            action = agent.get_action(state)
            assert env.action_space[0] <= action <= env.action_space[1]
